...
import { SIDE_MENU, MAIN_HOME_SCREEN } from "../navigation";
...
const CommonService = {
  ...
  openTripSearchListFromDeepLink: ({
    id,
    carpoolGetTripDetails,
    carpoolDriverGetTripDetails,
    carpoolGetSearchTrips,
    ...
    if (id) {
      id = id.substring(4, id.indexOf("A73"));
      let tripDetailsApiCall = carpoolGetTripDetails;
      if (isDriver) {
        tripDetailsApiCall = carpoolDriverGetTripDetails;
      }
      tripDetailsApiCall({
        callback: (res) => {
          try {
            console.log("deep links res", res);
            if (res.success && res.data && res.data.status == "success") {
              let trip = res.data.result;
              if (trip) {
                let origin = trip.destinations[0];
                let dest = trip.destinations[trip.destinations.length - 1];
                let data = {
                  pickup_lat: origin.lat,
                  pickup_long: origin.long,
                  destination_lat: dest.lat,
                  destination_long: dest.long,
                  pickup_time: DateService.convertToUTC(
                    trip.start_date + "T" + "00:00"
                  ),
                  pickup_date: trip.start_date,
                  total_seat: 1,
                  noenc: true,
                };
                let noTripsData = {
                  pickup: origin.name,
                  drop: dest.name,
                  time: CommonService.getCurrentTimeUTC(),
                };
                props.carpoolSetPickupLocation({
                  location: {
                    latitude: origin.lat,
                ...
                if (isDriver) {
                  ...
                  setTimeout(() => {
                    componentId = global.homeComponentId;
                    carpoolGetSearchTrips({
                      data: data,
                      callback: (result) =>
                        CommonService.onResponseFromDeepLinkApiCall({
                          result,
                          carpoolSetSearchTrips,
                          screenName,
                          noResultScreenName,
  ...
  convertUTCDateToLocalTime: (unixTime) => {
    ...
    // console.log("raq unix time-----------", unixTime);
    var theDate = new Date(
      unixTime + new Date().getTimezoneOffset() * 60 * 1000
    );

    // console.log(
    //   "theDate- time zone difffff-----------",
  ...
  convertNumberToTime: (num) => {
    var hours = CommonService.preZero(Math.floor(num / 60));
    var minutes = CommonService.preZero(Math.floor(num % 60));
    var seconds = CommonService.preZero(Math.floor((Math.abs(num) * 60) % 60));
  ...
  removeQuotes: (text) => {
    return text.substr(0, Number(text.toString().length) - 1);
  },

  convertTimeStringToTime: (timeString, driftValue) => {
    // console.log("Date to convert", timeString);
    var target = new Date(timeString);
    // console.log("target date created is", target);
    return target.getTime() + Number(driftValue);
    // return target.getTime() / 1000 + Number(driftValue);
  },
